...
class nnUNetDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        if case_identifiers is None:
            # print(f'to get the key , the folder is {folder}')
            case_identifiers = get_case_identifiers(folder)

        case_identifiers.sort()
        # with open('debug.txt', 'a') as file:
        #     file.write(f'the number of case is {keys_number}\n')
        # case_identifiers = ['lymphdet_000', 'lymphdet_004'] #* 3. xjl add for overfit a data
        # print(f'the case_identifiers is {case_identifiers}')
        ...
        if len(case_identifiers) <= num_images_properties_loading_threshold:
            for i in self.dataset.keys():
                # print(f'the key is {i}', flush=True)
