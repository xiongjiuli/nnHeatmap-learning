...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        seg = configuration_manager.resampling_fn_data(seg, new_shape, original_spacing, target_spacing)
        if self.verbose:
        ...
        if has_seg:
            ...
            seg_node = np.where(seg > 0.5, 1, 0)
            properties['class_locations'] = self._sample_foreground_locations(seg_node, collect_for_this,
    ...
    def modify_seg_fn(self, seg: np.ndarray, plans_manager: PlansManager, dataset_json: dict,
        ...
        if max_val - min_val != 0:
            seg = seg
            # seg = (seg - min_val) / (max_val - min_val) # xjl change
        else:
            # 如果最大值和最小值相等，数组中所有元素都是相同的
            # 在这种情况下，可以将数组设置为全0（因为没有变化范围）
...
def example_test_case_preprocessing():
    ...
    plans_file = '/public_bme/data/xiongjl/nnDet/DataFrame/nnUNet_preprocessed/Dataset502_lymphdet/nnUNetPlans.json'
    dataset_json_file = '/public_bme/data/xiongjl/nnDet/DataFrame/nnUNet_preprocessed/Dataset502_lymphdet/dataset.json'
    input_images = ["/public_bme/data/xiongjl/nnDet/DataFrame/nnUNet_raw/Dataset502_lymphdet/imagesTr/lymphdet_954_0000.nii.gz", ]  # if you only have one channel, you still need a list: ['case000_0000.nii.gz']

    configuration = '3d_fullres'
    pp = DefaultPreprocessor()
    seg_file = ["/public_bme/data/xiongjl/nnDet/DataFrame/nnUNet_raw/Dataset502_lymphdet/labelsTr/lymphdet_954.nii.gz", ]
    ...
    data, _, properties = pp.run_case(input_images, seg_file=seg_file, plans_manager=plans_manager,
